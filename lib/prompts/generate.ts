...
export const GENERATE_PROMPT_VERSION = "2026-06-25.3";
...
- safe_rewrite: a headline and primary_text that KEEP the same angle AND its persuasive hook - copy the buyer would actually run. Soften every trigger you flagged, and nothing else (reframe outcomes as guidance not guarantees, fix personal-attribute phrasing, drop unprovable specifics); do NOT collapse it into generic filler ("explore our solutions", "unlock potential"), and do NOT invent new claims, offers, or guarantees the ad did not make - a compliant but lifeless rewrite is a failure. When level is low, return a clean equivalent of the original.
