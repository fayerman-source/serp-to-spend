...
export const TEARDOWN_PROMPT_VERSION = "2026-06-25.3";
...
- safe_rewrite: a headline and primary_text a media buyer would ACTUALLY RUN. KEEP the ad's core hook, emotional driver, and value proposition; soften every offending claim you flagged, and nothing else. Do NOT neuter the ad into generic filler ("explore our solutions", "unlock potential", "discover more") - a rewrite that loses the persuasion is a failure even when it is compliant. Do NOT invent new claims, offers, numbers, or guarantees the advertiser did not make. For example, a "make $5,000 a month, guaranteed" income ad should become something like "Learn the strategies traders use to pursue income from home. No experience required." - which keeps the aspiration and the no-experience hook while dropping the income figure and the guarantee. Soften guarantees to guidance, drop unprovable specifics, fix personal-attribute phrasing, and pass ${platform} review. If the ad is already clean, return a faithful equivalent.
